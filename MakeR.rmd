...
```{r}
ds <- rast("Data/cnmems_SST_MED_SST_L4_REP_OBSERVATIONS_010_021_analysed_sst_1982_2024.nc.nc")
# Each layer name must be the corresponding date under YYYY-MM-DD format
time(ds) <- as.Date(time(ds))
names(ds) <- time(ds)
```
...
baseline_qt90 <- BEE.calc.baseline(rast_name = ds, start_date = "1991-01-01", end_date = "2020-12-31", threshold = "qt", quantile_value = 0.9, smooth_window = 7) 
...
baseline_qt90 <- BEE.calc.baseline(rast_name = ds, start_date = "1982-01-01", end_date = "2010-12-31", threshold = "qt", quantile_value = 0.9, time_window = 5, smooth_window = 7) 
baseline_qt10 <- BEE.calc.baseline(rast_name = ds, start_date = "1982-01-01", end_date = "2010-12-31", threshold = "qt", quantile_value = 0.1, time_window = 5, smooth_window = 7) 
baseline_mean <- BEE.calc.baseline(rast_name = ds, start_date = "1982-01-01", end_date = "2010-12-31", threshold = "mean", quantile_value = NULL, time_window = 5, smooth_window = 7)
...
```{r}
# 6 minutes s to run on 3 332 pixels and 10 227 days
gc()
...
points_metrics <- BEE.calc.metrics_point(Events_corrected, Values = ds, GPS, start_date = "2022-01-01", end_date = "2024-12-31", baseline_qt90, baseline_mean, group_by_event = TRUE)
...
All <- rbind(Frontignan,Frioul,Banyuls)
#plot
ggplot(All, aes(x = dates, y = max_anomaly_qt90, size = Nb_days, color = site)) +
  ...
  labs(title = "Caractérisation des vagues de chaleurs",
       x = "Dates",
       y = "Anomalie maximum (par rapport au 90eme percentile",
       size = "Nb_days",
       color = "Site") +
...
```{r}
list_morpho_metrics <- BEE.calc.metrics_morpho(Corrected_rasters, start_date = "2022-01-01", end_date = "2024-12-31") # 1 minutes 35 to run on 3 years
list_morpho_metrics_all_pixels <- list_morpho_metrics[[1]]
list_morpho_metrics_by_patch <- list_morpho_metrics[[2]]

# Calculate mean distance to escape from an extreme event
dist_to_escape <- BEE.calc.escape(Corrected_rasters, coords = GPS[1,c(1,2)], account_all=FALSE, start_date = "2022-01-01", end_date = "2024-12-31")
summary_dist <- dist_to_escape[[2]]
```
