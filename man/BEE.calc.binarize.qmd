...
BEE.calc.binarize <- function(rast_name, baseline, direction) {
  ...
  if(class(baseline) == "SpatRaster"){
    ...
    if (any(table(time(rast_name))!=1)){ #at least one date has several layers
      rep <- sum(table(time(rast_name)))/length(table(time(rast_name)))
      if (rep==round(rep)){# all dates are repeted the same number of time
        ...
        for (l in seq(1,rep,1)){ # create a list with all days from the same depths
          layers <- seq(l,n,rep) 
          rast_list[[l]]<-terra::rast(rast_name[[layers]])
        } 
        delta <- list() 
