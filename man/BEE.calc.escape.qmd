...
## Select timeframe of interest to save computation time later
  ...
  if (any(dist_dir$pixel_to_id=="no escape", na.rm = TRUE)){
    ...
    message("During the following dates, the raster was fully covered by an EE and it was no possible to compute a distance to escape or an azimut.", dangerous_date )
  }
  ...
  if (each_event==TRUE){
    ...
    ## Distance
    dist_dir[,distance_mean := mean(distance), by = ID]
    dist_dir[,distance_sd := sd(distance), by = ID]
    dist_dir[,distance_median := median(distance), by = ID]
    dist_dir[,distance_min := min(distance), by = ID]
    dist_dir[,distance_max := max(distance), by = ID]
    ## Since degree are a circular unit (after 360=0 comes 1,2...) we need a special way to compute it : 
    # Conversion to circular angle :
    dist_dir[, azimut_circ := circular::circular(azimut, units = "degrees", template = "geographics")]
    dist_dir[, azimut_mean := as.numeric(mean(azimut_circ)), by = ID]
    dist_dir[, azimut_med := if (any(is.na(azimut_circ))) NA_real_ else median(azimut_circ), by = ID] # not possible to compute median on NA + if I just put NA, it is going to expect logical instead of numerical value, that's why NA-real and not NA
    dist_dir[, azimut_sd := {
    ...
    dist_dir[,azimut_min := min(azimut), by = ID]
    dist_dir[,azimut_max := max(azimut), by = ID]
    
    # Delete column that refer to daily value and not to value compute on all the event :
    dist_dir[, c('date', 'distance', 'azimut', 'azimut_circ') := NULL]
