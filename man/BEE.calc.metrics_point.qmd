...
BEE.calc.metrics_point <- function(Events_corrected, Values, GPS, 
  ...
  metrics <- lapply(1:length(df_list), function(p) { # Go through each pixel
    ...
    # group_by(ID) %>%
    # summarise(
    #   max_category = levels(daily_category)[which.max(as.numeric(daily_category))],  # Catégorie avec la plus grande valeur
    #   .groups = 'drop'
    # )
    ...
    setnames(df, old = "max_category.y", new = "max_category")
    if (group_by_event) {
    df <- df %>%
      # Delete daily values that are not usefull to describe the full event
      select(-baseline_qt90, -baseline_mean, -anomaly_mean, -anomaly_qt90, -anomaly_unit, -daily_category, -date, -ID, -daily_category) %>%
      distinct(event_ID, .keep_all = TRUE)  # Une seule ligne par ID
    
  } else {
    df <- df %>% select(-date, -ID)
  }
    return(df)
  })
