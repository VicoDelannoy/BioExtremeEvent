...
# patches is mixing NA values from the land and NA values from outside of the patch, thus I'll replace NA values from the land by 999
  message("NA values provided have been replaced by '999', this is to prevent computational confusion between values that are NA and values created for pixel outside of patches durring compilation") 
  rasters[is.na(rasters)] <- 999
  patch_list <- lapply(rasters, FUN = function(rasters){
    patch <- patches(rasters, directions = 8, zeroAsNA = T, allowGaps = T)
    return(patch)
  })
    
  nb_999 <- global( rasters == 999, fun = "sum", na.rm = TRUE)
  ...
  dist_list <- lapply(patch_list, function(x) { # x <- patch_list[[1]]
    ...
    d <- time(x)
    # get the number of NA value that day
    nb_NA <- nb_999[which(as.Date(rownames(nb_999)) == time(x)),]
    
    data <- data.table(
      x = coordonates[,1],
      y = coordonates[,2],
      values = vals, # value of the pixel
      cell_size = cell_size,
    ...
    data[, cover_percent := n_pixel / (pixel_tot-nb_NA), by = patch_id]
    data[, core_pixel := n_pixel - length_boundary]
    data[, NA_pixel_patch := ifelse(n_pixel == nb_NA, "yes", "no"), by = patch_id]
    date <- time(x)
    data[, ID := paste0(.I, "_", date, "_", patch_id)]
    data[, pixel_id :=.I]
