...
#; coords = GPS[1,c(1,2)] ; account_all=FALSE
...
BEE.calc.escape(true_event_output, 
                start_date = NULL, end_date = NULL, 
                pixel = all, # a vector with x,y coordinates or a df of x,y coordinates or "all" if you want to comute distance metrics for all pixels
                account_all_days = "events_all"){ # TRUE = mean distance will be based on distances from all days in the time frame, "events_all" = compute one mean distance per events, including all days belonging to this event event if they are a '0' day, "events_1" compute one mean distance per events, excluding the days that belong to  an event are bellow thershold., FALSE = give on mean value for all days that are above thershold (1).

### Recreate a Spatraster using Events_corrected. In this list, there are one df per pixel and on raw per dates
## Get data and shell
...
## Select timeframe of interest to save computation time later
  ...
  for (i in seq_along(data)){
    ...
    if (any(data[[i]]$Cleanned_value == 0, na.rm=TRUE)) {
    event[which(data[[i]]$Cleanned_value == 0)] <- data[[i]]$Cleanned_value[which(data[[i]]$Cleanned_value == 0)]
    }
  }
}
```
