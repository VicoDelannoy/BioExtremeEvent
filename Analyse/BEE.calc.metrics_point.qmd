...
EE.calc.metrics_point <- function(Events_corrected, Values, GPS, start_date=NULL, end_date=NULL){
  ########################################## WARNINGS ############################################################
  ...
  #rename properly each raster from period_of_interst
  if (!is.null(GPS)){ #function for a GPS point or a dataframe with several GPS cooridantes
    warning("Please provide a dataframe with GPS position (in a format consitent with the one of your raster layers).")
  }
...
############### À ADAPTER DANS LE STYLE DU CODE JUSTE AU DESSUS :
    mean_value <- c()
    median_value <- c()
    max_value <- c()
    min_value <- c()
    for (p in 1:length(df)){
      print(p)
      for (l in 1:length(df[[p]]$first_day)){
        print(l)
      date <- as.character(as.Date(unlist(date_lists[[p]][l])))
      mean_value <- c(mean_value, mean(as.numeric(period_of_interest_values[p,date]), na.rm=TRUE))
      median_value <- c(median_value, median(as.numeric(period_of_interest_values[p,date]), na.rm=TRUE))
      max_value <- c(max_value, max(as.numeric(period_of_interest_values[p,date]), na.rm=TRUE))
      min_value <- c(min_value, min(as.numeric(period_of_interest_values[p,date]), na.rm=TRUE))
      }
    }
    
    warning("Shared 'eventnb' between different posistions (GPS_name) don't necessarily indicates that the same event encompassed the two GPS positions. 'eventnb' is proper to each GPS positions.")
    return(df)
  # For the all period I want : Total nb of day, Frequency, maximum intensity, mean and median intensity, sum of anomalies, date of maximum intensity, first date 1, last date 1
}
```
